...
export default function SouthDakotaPage() {
  const stPiers = useMemo(() => unified.filter((r) => r.state === "SD"), []);
  ...
  return (
    <div>
      ...
      <section className="py-16 md:py-24 text-center px-4 bg-cream" style={{ backgroundImage: "radial-gradient(circle at 20% 80%, rgba(10,61,98,0.06) 0%, transparent 50%)" }}>
        ...
        <p className="text-gray-500 mt-4 max-w-lg mx-auto">{stPiers.length}+ fishing piers across South Dakota. Saltwater, freshwater, and river access. {namedCount} named piers with details.</p>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-4 pt-10 pb-2">
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
          <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-3">Pier Fishing in South Dakota</h2>
          <p className="text-gray-600 leading-relaxed text-sm">South Dakota offers {stPiers.length.toLocaleString()}+ fishing piers, docks, and jetties across the state. From Lake Oahe, Lake Sharpe, and the Missouri River, there&apos;s no shortage of places to cast a line. Popular catches include walleye, bass, and catfish.</p>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
          <h3 className="font-[Cabin] font-bold text-ocean mb-3">Tips for Pier Fishing in South Dakota</h3>
          <ul className="space-y-2 text-sm text-gray-700">
            <li className="flex items-start gap-2"><span className="text-ocean mt-0.5">&#10003;</span> Check South Dakota&apos;s fishing license requirements &mdash; most states require one even for pier fishing.</li>
        ...
        <section className="max-w-6xl mx-auto px-4 pt-8 pb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {cityMap.slice(0, 16).map(([city, count]) => (
              <Link key={city} href={`/cities/south-dakota-${city.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}`} className="text-left bg-white border border-gray-200 rounded-lg p-3 hover:border-ocean hover:shadow-sm transition"><p className="font-bold text-charcoal text-sm">{city}</p><p className="text-gray-400 text-xs">{count} pier{count !== 1 ? "s" : ""}</p></Link>
            ))}
          </div>
        </section>
      )}

      <div id="pier-list">{selectedCity && <div className="max-w-6xl mx-auto px-4 pb-4"><button onClick={() => setSelectedCity(null)} className="text-sm text-ocean hover:underline">&larr; Show all {stPiers.length} piers</button></div>}</div><PierList piers={filteredPiers} stateName="South Dakota" />

      <section className="max-w-4xl mx-auto px-4 py-10">
        <h2 className="font-[Cabin] text-2xl font-bold text-charcoal mb-4">South Dakota Pier Fishing FAQ</h2>
        <div className="space-y-2">
          {[
            { q: "How many fishing piers are in South Dakota?", a: `PierSeeker lists ${stPiers.length}+ fishing piers, docks, and jetties across South Dakota.` },
            { q: "Do I need a fishing license in South Dakota?", a: "Most states require a fishing license even when fishing from a public pier. Check South Dakota's fish and wildlife department for current requirements." },
            { q: "What fish can I catch from piers in South Dakota?", a: "Popular catches from South Dakota piers include walleye, bass, and catfish. Species vary by location and season." },
            { q: "Are fishing piers in South Dakota free?", a: "Many fishing piers in South Dakota are free to access. Some popular piers may charge a small fee during peak season." },
