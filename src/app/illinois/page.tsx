...
export default function IllinoisPage() {
  const stPiers = useMemo(() => unified.filter((r) => r.state === "IL"), []);
  ...
  return (
    <div>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        mainEntity: [
          { "@type": "Question", name: "How many fishing piers are in Illinois?", acceptedAnswer: { "@type": "Answer", text: `There are ${stPiers.length} fishing piers in Illinois. PierSeeker has mapped every public pier, dock, and jetty across the state.` } },
          { "@type": "Question", name: "Do I need a fishing license to fish from a pier in Illinois?", acceptedAnswer: { "@type": "Answer", text: "Most states require a fishing license even when fishing from a public pier. Check Illinois's fish and wildlife department for current license requirements and fees." } },
          { "@type": "Question", name: "Are there free fishing piers in Illinois?", acceptedAnswer: { "@type": "Answer", text: "Many fishing piers in Illinois are free to access, especially those at public parks and state recreation areas. Some popular piers may charge a small access fee." } },
          { "@type": "Question", name: "What fish can I catch from piers in Illinois?", acceptedAnswer: { "@type": "Answer", text: "Species vary by location and season. Coastal piers in Illinois often yield redfish, flounder, and trout. Freshwater piers are great for bass, catfish, and crappie." } },
          { "@type": "Question", name: "How do I find fishing piers near me in Illinois?", acceptedAnswer: { "@type": "Answer", text: `Use PierSeeker to browse all ${stPiers.length} fishing piers in Illinois by city. Each listing includes a map, GPS coordinates, and directions.` } },
      ...
      <section className="py-16 md:py-24 text-center px-4 bg-cream" style={{ backgroundImage: "radial-gradient(circle at 20% 80%, rgba(10,61,98,0.06) 0%, transparent 50%)" }}>
        <p className="text-ocean text-sm font-bold tracking-wider uppercase mb-3 font-[Cabin]">Illinois Fishing Pier Directory</p>
        <h1 className="font-[Cabin] text-4xl md:text-5xl font-bold text-charcoal leading-tight max-w-3xl mx-auto">Every Fishing Pier in Illinois</h1>
        <p className="text-gray-500 mt-4 max-w-lg mx-auto">{stPiers.length}+ fishing piers across Illinois. Saltwater, freshwater, and river access. {namedCount} named piers with details.</p>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-4 pt-10 pb-2">
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
          <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-3">Pier Fishing in Illinois</h2>
          <p className="text-gray-600 leading-relaxed text-sm">Illinois offers {stPiers.length.toLocaleString()}+ fishing piers, docks, and jetties across the state. From Lake Michigan and the Illinois River, there&apos;s no shortage of places to cast a line. Popular catches include salmon, perch, and catfish.</p>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
          <h3 className="font-[Cabin] font-bold text-ocean mb-3">Tips for Pier Fishing in Illinois</h3>
          <ul className="space-y-2 text-sm text-gray-700">
            <li className="flex items-start gap-2"><span className="text-ocean mt-0.5">&#10003;</span> Check Illinois&apos;s fishing license requirements &mdash; most states require one even for pier fishing.</li>
      ...
      <div id="pier-list">{selectedCity && <div className="max-w-6xl mx-auto px-4 pb-4"><button onClick={() => setSelectedCity(null)} className="text-sm text-ocean hover:underline">&larr; Show all {stPiers.length} piers</button></div>}</div><PierList piers={filteredPiers} stateName="Illinois" />

      <section className="max-w-4xl mx-auto px-4 py-10">
        <h2 className="font-[Cabin] text-2xl font-bold text-charcoal mb-4">Illinois Pier Fishing FAQ</h2>
        <div className="space-y-2">
          {[
            { q: "How many fishing piers are in Illinois?", a: `PierSeeker lists ${stPiers.length}+ fishing piers, docks, and jetties across Illinois.` },
            { q: "Do I need a fishing license in Illinois?", a: "Most states require a fishing license even when fishing from a public pier. Check Illinois's fish and wildlife department for current requirements." },
            { q: "What fish can I catch from piers in Illinois?", a: "Popular catches from Illinois piers include salmon, perch, and catfish. Species vary by location and season." },
            { q: "Are fishing piers in Illinois free?", a: "Many fishing piers in Illinois are free to access. Some popular piers may charge a small fee during peak season." },
