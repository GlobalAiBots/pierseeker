...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const title = `${cat.title} in America | PierSeeker`;
  const description = `${cat.totalCount.toLocaleString()} ${cat.title.toLowerCase()} across ${cat.states.length} states. ${cat.description}`;
  return {
    title,
    description,
