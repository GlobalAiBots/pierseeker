...
export default function MainePage() {
  const stPiers = useMemo(() => unified.filter((r) => r.state === "ME"), []);
  ...
  return (
    <div>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        mainEntity: [
          { "@type": "Question", name: "How many fishing piers are in Maine?", acceptedAnswer: { "@type": "Answer", text: `There are ${stPiers.length} fishing piers in Maine. PierSeeker has mapped every public pier, dock, and jetty across the state.` } },
          { "@type": "Question", name: "Do I need a fishing license to fish from a pier in Maine?", acceptedAnswer: { "@type": "Answer", text: "Most states require a fishing license even when fishing from a public pier. Check Maine's fish and wildlife department for current license requirements and fees." } },
          { "@type": "Question", name: "Are there free fishing piers in Maine?", acceptedAnswer: { "@type": "Answer", text: "Many fishing piers in Maine are free to access, especially those at public parks and state recreation areas. Some popular piers may charge a small access fee." } },
          { "@type": "Question", name: "What fish can I catch from piers in Maine?", acceptedAnswer: { "@type": "Answer", text: "Species vary by location and season. Coastal piers in Maine often yield redfish, flounder, and trout. Freshwater piers are great for bass, catfish, and crappie." } },
          { "@type": "Question", name: "How do I find fishing piers near me in Maine?", acceptedAnswer: { "@type": "Answer", text: `Use PierSeeker to browse all ${stPiers.length} fishing piers in Maine by city. Each listing includes a map, GPS coordinates, and directions.` } },
      ...
      <section className="py-16 md:py-24 text-center px-4 bg-cream" style={{ backgroundImage: "radial-gradient(circle at 20% 80%, rgba(10,61,98,0.06) 0%, transparent 50%)" }}>
        <p className="text-ocean text-sm font-bold tracking-wider uppercase mb-3 font-[Cabin]">Maine Fishing Pier Directory</p>
        <h1 className="font-[Cabin] text-4xl md:text-5xl font-bold text-charcoal leading-tight max-w-3xl mx-auto">Every Fishing Pier in Maine</h1>
        <p className="text-gray-500 mt-4 max-w-lg mx-auto">{stPiers.length}+ fishing piers across Maine. Saltwater, freshwater, and river access. {namedCount} named piers with details.</p>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-4 pt-10 pb-2">
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
          <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-3">Pier Fishing in Maine</h2>
          <p className="text-gray-600 leading-relaxed text-sm">Maine offers {stPiers.length.toLocaleString()}+ fishing piers, docks, and jetties across the state. From the Atlantic coast, Moosehead Lake, and Sebago Lake, there&apos;s no shortage of places to cast a line. Popular catches include striped bass, landlocked salmon, and trout.</p>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
          <h3 className="font-[Cabin] font-bold text-ocean mb-3">Tips for Pier Fishing in Maine</h3>
          <ul className="space-y-2 text-sm text-gray-700">
            <li className="flex items-start gap-2"><span className="text-ocean mt-0.5">&#10003;</span> Check Maine&apos;s fishing license requirements &mdash; most states require one even for pier fishing.</li>
        ...
        <section className="max-w-6xl mx-auto px-4 pt-8 pb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {cityMap.slice(0, 16).map(([city, count]) => (
              <Link key={city} href={`/cities/maine-${city.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}`} className="text-left bg-white border border-gray-200 rounded-lg p-3 hover:border-ocean hover:shadow-sm transition"><p className="font-bold text-charcoal text-sm">{city}</p><p className="text-gray-400 text-xs">{count} pier{count !== 1 ? "s" : ""}</p></Link>
            ))}
          </div>
        </section>
      )}

      <div id="pier-list">{selectedCity && <div className="max-w-6xl mx-auto px-4 pb-4"><button onClick={() => setSelectedCity(null)} className="text-sm text-ocean hover:underline">&larr; Show all {stPiers.length} piers</button></div>}</div><PierList piers={filteredPiers} stateName="Maine" />

      <section className="max-w-4xl mx-auto px-4 py-10">
        <h2 className="font-[Cabin] text-2xl font-bold text-charcoal mb-4">Maine Pier Fishing FAQ</h2>
        <div className="space-y-2">
          {[
            { q: "How many fishing piers are in Maine?", a: `PierSeeker lists ${stPiers.length}+ fishing piers, docks, and jetties across Maine.` },
            { q: "Do I need a fishing license in Maine?", a: "Most states require a fishing license even when fishing from a public pier. Check Maine's fish and wildlife department for current requirements." },
            { q: "What fish can I catch from piers in Maine?", a: "Popular catches from Maine piers include striped bass, landlocked salmon, and trout. Species vary by location and season." },
            { q: "Are fishing piers in Maine free?", a: "Many fishing piers in Maine are free to access. Some popular piers may charge a small fee during peak season." },
