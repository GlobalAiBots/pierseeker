...
export default function EmailCapture() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!email) return;
    window.location.href = `mailto:newsletter@pierseeker.com?subject=Subscribe&body=Please add ${email} to the PierSeeker newsletter`;
    setDone(true);
  }
  ...
  return (
    <section className="max-w-2xl mx-auto px-4 pb-12">
      <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
        <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-1">Get Fishing Updates</h2>
        <p className="text-gray-500 text-sm mb-5">New piers, fishing reports, and pier fishing tips from across America. No spam, unsubscribe anytime.</p>
        {!done ? (
