...
export default function PierList({ piers, stateName }: { piers: UnifiedPier[]; stateName: string }) {
  const [showAll, setShowAll] = useState(false);
  const sorted = useMemo(() => sortPiers(piers), [piers]);
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 pt-4 pb-8">
      <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-4">All {piers.length} {stateName} Fishing Piers</h2>
      ...
      {namedDisplay.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
          {namedDisplay.map((r) => (
            <Link key={r.id} href={`/piers/${r.id}`} className="group block bg-white border border-gray-200 rounded-lg p-3 border-l-4 border-l-ocean shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
              <span className="font-[Cabin] font-bold text-charcoal group-hover:text-ocean transition text-sm">{r.name}</span>
      ...
      {!showAll && (named.length > 30 || generic.length > 20) && (
        <button onClick={() => setShowAll(true)} className="mt-4 w-full py-3 bg-white border border-gray-200 rounded-lg text-sm font-semibold text-ocean hover:bg-ocean/5 transition">
          Show all {piers.length} piers ({named.length - namedDisplay.length + generic.length - genericDisplay.length} more)
        </button>
      )}
    </section>
  );
